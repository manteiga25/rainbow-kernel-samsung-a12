	...
	while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		...
		if (pwr_status == IMGSENSOR_HW_POWER_STATUS_ON &&
		   ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
			...
			PK_INFO("[power on]sensor_idx = %d, pin=%d, pin_state_on=%d, hw_id =%d\n",
					sensor_idx, ppwr_info->pin,
					ppwr_info->pin_state_on, psensor_pwr->id[ppwr_info->pin]);
		...
		while (pin_cnt) {
			...
			if (ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
				...
				PK_INFO("[power off]sensor_idx = %d, pin=%d, pin_state_off=%d, hw_id =%d\n",
						sensor_idx, ppwr_info->pin,
						ppwr_info->pin_state_off, psensor_pwr->id[ppwr_info->pin]);
